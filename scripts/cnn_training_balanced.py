...
def run_training_balanced(csv_path, img_dir, out_dir):
    ...
    sample_weights = np.array([class_weights[1] if l==1 else class_weights[0] for l in train_labels])
    sampler = WeightedRandomSampler(sample_weights, num_samples=len(sample_weights), replacement=True)

    dataloaders = {
        "train": DataLoader(train_ds, batch_size=BATCH_SIZE, sampler=sampler, num_workers=4),
