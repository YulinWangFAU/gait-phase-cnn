# -*- coding: utf-8 -*-
"""
generate_heatmaps_auto_split_all_balanced.py
--------------------------------------------
Balanced subject-wise split version of heatmap generation for GaitPDB dataset.

Differences from original:
- Uses subject-wise *balanced* splitting (Co/Pt split separately before merge).
- Output directories and CSV names include '_balanced' suffix for easy comparison.
"""
...
OUT_BASE = "/home/woody/iwi5/iwi5325h/gaitphasecnn_middle_data_balanced"
...
def split_subjects_balanced(subjects):
    """Split subjects (Co/Pt) into balanced train/val/test groups."""
    co_subj = [s for s in subjects if "Co" in s]
    pt_subj = [s for s in subjects if "Pt" in s]

    random.shuffle(co_subj)
    random.shuffle(pt_subj)

    def split_group(subj_list):
        n = len(subj_list)
        n_train = int(TRAIN_RATIO * n)
        n_val = int(VAL_RATIO * n)
        train = subj_list[:n_train]
        val = subj_list[n_train:n_train + n_val]
        test = subj_list[n_train + n_val:]
        return train, val, test

    train_co, val_co, test_co = split_group(co_subj)
    train_pt, val_pt, test_pt = split_group(pt_subj)

    train = train_co + train_pt
    val = val_co + val_pt
    test = test_co + test_pt
...
# ---------- Main Process ----------
def main():
    print("🚀 Starting balanced heatmap generation...")
    ...
    for EXPERIMENT in EXPERIMENTS:
        ...
        for TASK_MODE in TASK_MODES:
            ...
            subjects = sorted(list(set([f[0:6] for f in selected_files])))
            ...
            for METHOD in METHODS:
                for SIGNAL_TYPE in SIGNAL_TYPES:
                    OUT_DIR = os.path.join(
                        OUT_BASE,
                        f"heatmaps_{METHOD}_{SIGNAL_TYPE}_σ{SIGMA}_i{INTERP_POINTS}_{EXPERIMENT}_{TASK_MODE}_balanced"
                    )
                    CSV_PATH = os.path.join(
                        OUT_BASE,
                        f"labels_{METHOD}_{SIGNAL_TYPE}_σ{SIGMA}_i{INTERP_POINTS}_{EXPERIMENT}_{TASK_MODE}_balanced.csv"
                    )
                    ...
                    print(f"✅ [{EXPERIMENT}-{TASK_MODE}-{METHOD}-{SIGNAL_TYPE}-balanced] "
