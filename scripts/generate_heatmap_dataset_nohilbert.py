...
All output folders and files are marked with "_nohilbert"
"""
...
output_dir = os.path.join(Config.HEATMAP_DIR + "_nohilbert")  # 🟢 独立的输出文件夹
...
def get_heat(signal):
    ...
    # === Use derivative instead of Hilbert transform ===
    dy = np.gradient(signal)
    signal = (signal - np.mean(signal)) / np.max(np.abs(signal))
    dy = dy / np.max(np.abs(dy))

    # === Construct (signal, derivative) phase points ===
    points = np.vstack([signal, dy]).T
    points = inter2D(points)
    ...
    # === Padding to prevent border cutoff ===
    pad = 32
    p = int(pad / 2)
...
# === Main Loop ===
print(f"\n🧭 Running Heatmap Generation (NO Hilbert)")
...
for index_file in INDEX_FILES:
    ...
    for i, row in tqdm(df.iterrows(), total=len(df), desc=f"{group_name}"):
        ...
        try:
            ...
            out_name = f"{basename}_nohilbert.png"
